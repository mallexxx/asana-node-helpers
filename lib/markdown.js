...
function markdownToAsanaHtml(markdown) {
    ...
    // Convert markdown links to Asana user profiles to proper mention format
    // [Name](https://app.asana.com/0/profile/USER_GID) -> <a data-asana-gid="USER_GID" data-asana-type="user">Name</a>
    ...
    // Convert bare user profile URLs to proper mention format (use GID as display text)
    markdown = markdown.replace(/https:\/\/app\.asana\.com\/0\/profile\/(\d+)/g, (match, gid) => {
        const placeholder = `ASANA-MENTION-PLACEHOLDER-${asanaReplacements.length}-END`;
        asanaReplacements.push({
            placeholder,
    ...
    // Protect other Asana URLs (tasks, projects) from being converted to <a> tags
    markdown = markdown.replace(/(https:\/\/app\.asana\.com\/0\/\d+\/[^\s<>]+)/g, (match) => {
        const placeholder = `ASANA-URL-PLACEHOLDER-${asanaReplacements.length}-END`;
        asanaReplacements.push({
            placeholder,
            replacement: match
        });
