...
async function getTask(tasksApiInstance, taskGid, options = {}) {
    try {
        const opts = {
            opt_fields: options.opt_fields || 'name,completed,due_on,due_at,notes,assignee.name,projects.name,tags.name',
            ...options
        };
 ...
 */
async function updateTask(tasksApiInstance, taskGid, updates) {
    try {
        const result = await tasksApiInstance.updateTask({ data: updates }, taskGid);
        return result.data;
    } catch (error) {
