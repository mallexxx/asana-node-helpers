...
async function createTask(tasksApiInstance, taskData) {
    try {
        ...
        // If this was an HTML-related error, log the HTML for debugging
        if (taskData.html_notes && error.message?.includes('XML')) {
            console.error('Generated HTML that caused error:', taskData.html_notes);
        }
...
async function updateTask(tasksApiInstance, taskGid, updates, options = {}) {
    try {
        ...
        if (processedUpdates.html_notes && error.message?.includes('XML')) {
            console.error('Generated HTML that caused error:', processedUpdates.html_notes);
        }
    ...
    try {
        ...
        do {
            ...
            if (allTasks.length >= (searchOptions.maxResults || 500)) {
                const isMCP = process.argv[1]?.includes('mcp-server.js');
                if (!isMCP) {
                    console.error(`[Search] Limiting results to ${allTasks.length} tasks`);
                }
                break;
            }
        } while (offset);
