...
function loadCache(workspace) {
    try {
        ...
        console.log(`[Cache] Using cached projects (${cacheData.projects.length} projects)`);
        return cacheData.projects;
    } catch (error) {
        console.log('[Cache] Failed to load cache:', error.message);
        return null;
    }
}
...
function saveCache(workspace, projects) {
    try {
        ...
        console.log(`[Cache] Saved ${projects.length} projects to cache`);
    } catch (error) {
        console.log('[Cache] Failed to save cache:', error.message);
    }
}
...
async function searchProjects(projectsApiInstance, workspace, options = {}) {
    try {
        ...
        if (!allProjects) {
            console.log('[API] Fetching projects from Asana...');
            ...
            console.log(`[API] Fetched ${allProjects.length} projects`);
