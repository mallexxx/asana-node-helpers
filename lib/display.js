...
function displayTaskDetails(task, options = {}) {
    ...
    if (noteFormat === 'html' && task.html_notes) {
        ...
        let readableNotes = task.html_notes
            ...
            .replace(/<a[^>]*data-asana-gid="(\d+)"[^>]*data-asana-type="user"[^>]*>(.*?)<\/a>/g, (match, gid, name) => {
                // Strip @ from name if present, create markdown link with /profile/ URL
                const cleanName = name.replace(/^@+/, '');
                return `[${cleanName}](https://app.asana.com/0/profile/${gid})`;
            })
            .replace(/<a[^>]*data-asana-type="user"[^>]*data-asana-gid="(\d+)"[^>]*>(.*?)<\/a>/g, (match, gid, name) => {
                // Handle case where data-asana-type comes before data-asana-gid
                const cleanName = name.replace(/^@+/, '');
                return `[${cleanName}](https://app.asana.com/0/profile/${gid})`;
            })
            ...
            .replace(/&gt;/g, '>')
            .replace(/&nbsp;/g, ' ')
            // Normalize NO-BREAK SPACE (U+00A0) to regular space for editor compatibility
            .replace(/\u00a0/g, ' ')
            // Clean up excessive newlines and spaces
            .replace(/\n{3,}/g, '\n\n')
            // Remove extra newline between headers and lists (header already has trailing \n, list adds another)
            .replace(/(^|\n)(#{1,2}\s+.*)\n\n(-\s+)/gm, '$1$2\n$3')
            .replace(/\|\s+\|/g, '| ')
            .trim();
...
/**
 * Convert Asana HTML notes to markdown format
 * @param {string} htmlNotes - HTML notes from Asana
 * @returns {string} - Markdown formatted text
 */
function convertHtmlToMarkdown(htmlNotes) {
    if (!htmlNotes) return '';
