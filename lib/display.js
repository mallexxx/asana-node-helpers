...
function displayTaskDetails(task, options = {}) {
    ...
    if (noteFormat === 'html' && task.html_notes) {
        // Show raw HTML
        console.log(`\nNotes (HTML):\n${task.html_notes}`);
    } else if (noteFormat === 'text' && task.notes) {
        // Show plain text (stripped)
        console.log(`\nNotes (Plain Text):\n${task.notes}`);
    } else if (task.html_notes) {
        // Default: Convert HTML to readable markdown-like text for terminal display
        let readableNotes = task.html_notes
            ...
            // Convert links - preserve @mentions and regular links
