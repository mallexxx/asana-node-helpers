...
function displayTaskDetails(task, options = {}) {
    ...
    if (noteFormat === 'html' && task.html_notes) {
        ...
        let readableNotes = task.html_notes
            ...
            .replace(/<a[^>]*href="([^"]*)"[^>]*data-asana-type="user"[^>]*>(.*?)<\/a>/g, (match, href, name) => {
                // Strip @ from name if present, create markdown link
                const cleanName = name.replace(/^@+/, '');
                return `[${cleanName}](${href})`;
            })
            .replace(/<a[^>]*data-asana-type="user"[^>]*href="([^"]*)"[^>]*>(.*?)<\/a>/g, (match, href, name) => {
                // Handle case where data-asana-type comes before href
                const cleanName = name.replace(/^@+/, '');
                return `[${cleanName}](${href})`;
            })
